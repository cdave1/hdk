...
void RefreshLevelList()
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		levelList->add_item((int)levelEditorController->GetCurrentTotemWorld()->GetLevels()[i], levelName);
	}
	levelList->set_int_val((int)levelEditorController->GetCurrentLevel());
}
...
void Keyboard(unsigned char key, int x, int y)
{
	...
	switch (key)
	{
		...
		case 'b':
		case 'B':
			if (GLUTmodifiers == (GLUT_ACTIVE_CTRL | GLUT_ACTIVE_SHIFT))
			{
...
*/	 
	...
	m_worldFilenames = hdGetFileNames("Worlds");
	...
	m_textureFilenames = hdGetFileNames("/Textures");
