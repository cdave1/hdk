...
//DEFINE_HEAP(hdPVRTCLoader, "Textures");

hdPVRTCLoader::hdPVRTCLoader(){};
hdPVRTCLoader::~hdPVRTCLoader(){};
...
//
unsigned int  hdPVRTCLoader::LoadPartialTextureFromPVR(const char * const filename,
...
// 
unsigned int  hdPVRTCLoader::LoadTextureFromPVR(const char * const filename, GLuint * const texName, const void *psTextureHeader)
{
...
// 
unsigned int  hdPVRTCLoader::LoadDecompressedPartialTextureFromPVR(const char* const filename, unsigned int nLoadFromLevel, GLuint *const texName, const void *psTextureHeader)
{
	CPVRTResourceFile TexFile(filename);
	if (!TexFile.IsOpen()) return 0;
...
// 
unsigned int  hdPVRTCLoader::LoadDecompressedTextureFromPVR(const char* const filename, GLuint *texName, const void *psTextureHeader)
{
...
// 
unsigned int  hdPVRTCLoader::LoadDecompressedTextureFromPointer(const void* pointer, GLuint *texName, const void *psTextureHeader)
{
...
// Release texture by calling  ReleaseTexture.  Decompresses to RGBA8888 internally.
// 
unsigned int  hdPVRTCLoader::LoadDecompressedPartialTextureFromPointer(const void *pointer,
...
//
unsigned int hdPVRTCLoader::LoadPartialTextureFromPointer(const void * const pointer,
...
//
//
unsigned int hdPVRTCLoader::LoadTextureFromPointer(const void* pointer, GLuint *const texName, const void *psTextureHeader)
{
	return LoadPartialTextureFromPointer(pointer, 0, 0, texName, psTextureHeader);
}
...
//
void hdPVRTCLoader::ReleaseTexture(GLuint texName)
{
...
void hdPVRTCLoader::PVRTCDecompress(const void *pCompressedData,
							   const int Do2bitMode,
							   const int XDim,
							   const int YDim,
							   unsigned char* pResultImage)
...
//					
int EThdPVRTCLoaderDecompress(const void * const pSrcData, const int &x, const int &y, const void *pDestData,const int &nMode)
{
				  ...
				  const int &nMode)
{
	...
	{	// decompress into a buffer big enought to take the minimum size
		...
		i32read = EThdPVRTCLoaderDecompress(pSrcData,_MAX(x,ETC_MIN_TEXWIDTH),_MAX(y,ETC_MIN_TEXHEIGHT),pTempBuffer,nMode);
		...
		i32read = EThdPVRTCLoaderDecompress(pSrcData,x,y,pDestData,nMode);
