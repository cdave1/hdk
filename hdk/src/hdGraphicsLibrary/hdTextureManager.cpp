...
//	tex->name = name;
	...
	//  { GL_ALPHA, GL_ALPHA, GL_LUMINANCE_ALPHA, GL_RGB,
	...
	if( scaled_width == tex->width && scaled_height == tex->height )
	{
		...
		else
		{
			...
			{
				// Downsample fecking everything.
				tex->MinFilter = LinearMipMapNearest;
				tex->MagFilter = Nearest;
				
				downSample = (type != TT_Font && type != TT_ScaleableFont); //true;
			}
									 ...
									 GLenum format)
{
	...
	while(row++ < destHeight)
	{
		...
		while (col++ < destWidth)
		{
			...
			// Condense a 2x2 pixel square in source into 
			// a single pixel on dest. We need to do this
			// for each byte of the pixels, according to
			// the number of bytes per pixel. 
			// We don't care
			// too much about the format; this will work
			// whether the bytes are alpha, luminosity, RGB
			// RGBA, ABGR etc etc.
			while(byte++ < srcBPP)
			{
				pixelSum = (
				...
				*dest = (unsigned char)(pixelSum >> 2); // divide by 4
