...
void h2CirclePhysicsBody2(b2World* physicsWorld, b2Body** outBody, 
						 ...
						 bool isStatic, bool isSensor, int groupIndex, int categoryBits, int maskBits)
{
	if (radius <= B2_FLT_EPSILON)
	{
		hdPrintf("h2CirclePhysicsBody: Can't create circular physics body with a radius smaller than FLT_EPSILON", radius);
		*outBody = NULL;
		return;
	}
...
void hd2DMultiShapePhysicsBody(b2World* physicsWorld, b2Body** outBody, 
									const hdVec3& position, const hdVec3* vertices, const int vertexCount, 
									const float friction, const float density, const float restitution,
									bool isStatic, bool isSensor, int groupIndex, int categoryBits, int maskBits)
{
	b2BodyDef bd;
	...
	if (vertexCount <= 2)
	{
		hdPrintf("hd2DMultiShapeBodyFromVertices: Physics Body not created because the number of vertices specified (%d) was not in range", vertexCount);
		*outBody = NULL;
		return;
	}
								  ...
								  bool isStatic, bool isSensor, int groupIndex, int categoryBits, int maskBits)
{
	...
	if (vertexCount <= 2)
	{
		hdPrintf("hd2DTriangleStripPhysicsBody: Physics Body not created because the number of vertices specified (%d) was not in range", vertexCount);
		*outBody = NULL;
		return;
	}
									 ...
									 bool isStatic, bool isSensor, int groupIndex, int categoryBits, int maskBits)
{
	...
	if (vertexCount < 3 || vertexCount > b2_maxPolygonVertices) 
	{
		hdPrintf("hd2DBodyFromVertices: Physics Body not created because the number of vertices specified (%d) was not in range", vertexCount);
		*outBody = NULL;
		//*outShape = NULL;
		return;
	}
	...
	b2BodyDef bd;
	bd.position = b2Vec2(position.x, position.y); //position.x, position.y);
	*outBody = physicsWorld->CreateBody(&bd);
...
//	ComputeOBB(&_obb, _vertices, _vertexCount);
