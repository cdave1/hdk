...
public:
	...
	static string GetValueForKey(const char *key);
	
	static bool ConfigWasInitialized();
	


//	static double GetValueForKey(const char *key);

//	template<typename T> T GetValueForKey(const char *key);
	
	
};

/*
template<typename T> inline T hdConfig::GetValueForKey(const char *key);
{
	return (T)Config_GetValueForKey(key);
}
*/
