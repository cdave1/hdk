...
SmokeEmitter::SmokeEmitter(int maxParticles,
...
void SmokeEmitter::DoStartingTasks()
{
...
void SmokeEmitter::Step(hdTimeInterval elapsed)
{
...
void SmokeEmitter::MakeParticle(int pos)
{
    float Angle;
    float shade;
    hdParticle particle;
    ...
    shade = hdRandom(0.5f, 1.0f);
    particle.color.Setf(shade, shade, shade, 1.0f);
    particle.life = m_duration;
    particle.damping = hdRandom(0.2f, 0.8f);
    ...
    particle.direction.z = hdSinf(Angle)*(hdRandom(0.0f, m_initialParticleSpread));
...
const bool SmokeEmitter::IsFixedDuration() const
{
    return false;
}


void SmokeEmitter::ParticleAnimationFinished()
{
}
