...
{ 
...
#ifdef LEVEL_EDITOR
	m_blocks = new hdSerializablePointerList<Block, HDK_GAME_BlockMax>();
	m_jacks = new hdSerializablePointerList<Jack, HDK_GAME_JackMax>();
	m_joints = new hdSerializablePointerList<Joint, HDK_GAME_JointMax>();
	m_events = new hdSerializablePointerList<Event, HDK_GAME_EventMax>();
	
	m_backgroundLayer = new Layer(this, 200);
	m_midgroundLayer = new Layer(this, 100);
	m_foregroundLayer = new Layer(this, 75);	
#endif
	...
	m_sortedBlockCache = new hdTypedefList<Block*, HDK_GAME_BlockMax>();
...
void Level::Restart()
{
	m_blocks->Map(&Block::ResetVertices);
	m_jacks->Map(&Jack::ResetVertices);
	m_joints->Map(&Joint::ResetVertices);
}


void Level::ResetBlocks() const
{	
	m_blocks->Map(&Block::ResetVertices);	
	m_jacks->Map(&Jack::ResetVertices);
	m_joints->Map(&Joint::ResetVertices);

	m_blocks->Map(&Block::ResetPhysicsBody);	
	m_jacks->Map(&Jack::ResetPhysicsBody);
	
	// Parent objects well and truly gone, so we don't have
	// to delete anything to do with the joints...
	m_joints->Map(&Joint::CreateJoint);
}
...
void Level::InitGameObjects(hdGameWorld* gameWorld, hdOrthographicProjection* relativeProjection)
{
	this->Init(gameWorld);
	...
	{
		Block* block = (Block *)m_blocks->GetItems()[i];
		if (block != NULL)
		{
	...
	{
		Jack* jack = (Jack *)m_jacks->GetItems()[i];
		if (jack != NULL)
		{
			jack->Init(gameWorld);
		}
	...
	{
		Event* event = (Event *)m_events->GetItems()[i];
		if (event != NULL)
		{
...
void Level::ResetTextureCache()
{
	...
	for (int i = 0; i < m_blocks->GetItemCount(); ++i)
	{
		block = (Block *)m_blocks->GetItems()[i];
...
void Level::InitPhysicsObjects(hdGameWorld* gameWorld, b2World *physicsWorld)
{
	for (int i = 0; i < m_blocks->GetItemCount(); ++i)
	{
		Block* block = (Block *)m_blocks->GetItems()[i];
	...
	{
		Joint* item = (Joint *)m_joints->GetItems()[i];
		if (item != NULL)
		{
	...
	hdAABB currAABB = ((Level*)this)->GetAABB();
...
void Level::DestroyGameObjects() const
{	
	...
	m_blocks->Map(&Block::DestroyPhysicsBody);	
	m_jacks->Map(&Jack::DestroyPhysicsBody);
}


// Should really be a reduce function on the lists.
const hdAABB Level::GetAABB() 
{
	...
	for (int i = 0; i < m_blocks->GetItemCount(); ++i)
	{
		Block* block = (Block *)m_blocks->GetItems()[i];
		if (block != NULL && 
			(block->GetBlockType() != e_hdkBlockTypeCosmetic) &&
			(block->GetBlockType() != e_hdkBlockTypeBoundary) &&
			(block->GetBlockType() != e_hdkBlockTypeBillboard))
		{
...
void Level::CommitBlockStates() const
{
...
void Level::DestroyBlockTextures()
{
...
static hdTypedefList<Block*, 256>* m_blockPVS = NULL;
...
{
#ifdef LEVEL_EDITOR 
	m_blocks->Map(&Block::Step);
	m_jacks->Map(&Jack::Step);
	m_joints->Map(&Joint::Step);
	
	((Level *)this)->UpdateWaterLevel();
#else
	...
	if (m_blockPVS == NULL) m_blockPVS = new hdTypedefList<Block*, 256>();
	...
	{	
		...
		if (block->IsDrawable())
		{
			...
			if (block->ContainsGameFlag((uint32)e_hdkGameFlagsBlockDrawRaw))
			{
				block->m_frameStepRaw = true;
				++m_rawBlockCount;
			}
			else if (block->GetBlockType() == e_hdkBlockTypeBillboard)
			{
	...
	{
		block = m_blocks->GetItems()[i];
		if (block->ContainsGameFlag(e_hdkGameFlagsBlockDrawRaw))
		{
			block->Step();
		}
...
#ifdef LEVEL_EDITOR
void Level::DrawEditorView() const
{
	hdAABB currAABB;
	
	currAABB = ((Level*)this)->GetAABB();
 ...
 */
void Level::DrawBackgroundLayer(const hdTexture *texture, const float zDistance, 
...
void Level::DrawLevel(bool front) const
{
	Joint *joint;
	hdAABB scissorAABB;
...
void Level::DrawBillboards(bool front) const
{
	hdTexture* currentTexture;
	Block *block;
...
void Level::DrawPVSBlocksWithTexture(const bool front, hdTexture* texture) const
{
}
...
hdAABB Level::GetScissorAABB(bool front)
{
...
float Level::GetFloorDrawingLevel()
{
