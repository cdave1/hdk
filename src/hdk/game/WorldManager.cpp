...
static WorldManager* m_WorldManagerInstance = NULL;

WorldManager* WorldManager::Instance()
{
    if (m_WorldManagerInstance == NULL)
    {
        m_WorldManagerInstance = new WorldManager();
    }
    return m_WorldManagerInstance;
}


void WorldManager::TearDown()
{
    delete m_WorldManagerInstance;
    m_WorldManagerInstance = NULL;
}


WorldManager::WorldManager()
{
    m_cache = new hdTypedefList<WorldCache, 32>();
...
void WorldManager::ClearCache()
{
    hdPrintf("WorldManager: Clearing the cache...\n");
    while(m_cache->GetItemCount() > 0)
    {
        if (strlen(m_cache->GetItems()[0].bin) > 0)
        {
    ...
    hdPrintf("WorldManager: Finished clearing the cache...\n");
}


void WorldManager::SetCacheBinariesFlag(bool cacheBinaries)
{
...
World* WorldManager::FindTotemWorld(const char* filename)
{
    // look through cache, using filename:
    // - if match:
    //		- if World NULL, then assume binary is there and LoadTotemWorldFromUncompressBinary
    //			- there shouldn't be a match if m_cacheBinaries = false;
    //		- otherwise just return pointer to world
    // - if no match
    //		- call LoadTotemWorld
    World *tw = NULL;
    ...
    for (int i = 0; i < m_cache->GetItemCount(); i++)
    {
        ...
        {
            hdPrintf("[WorldManager::FindTotemWorld] File found in cache out of %d items...", m_cache->GetItemCount());
            ...
            {
                hdPrintf("Returning cached world\n");
...
bool WorldManager::BackupTotemWorldFile(const char *path)
{
    char * buffer;
    long size;
    char backupPath[256];
...
bool WorldManager::SaveTotemWorld(const World *tw, const char *path)
{
...
int WorldManager::SaveTotemWorldBinary(const World *tw, const char *path)
{
    /* std::iostream stuff */
    char *ossBuffer;
    long ossLen;
    ...
    if (strlen(path) == 0)
    {
        hdPrintf("World::SaveTotemWorld error: empty file path \n");
        return -1;
    }
...
World * WorldManager::LoadTotemWorldFromText(const char* filename)
{
    char *txtBuf;
    long txtLen, readLen;
    ...
    hdPrintf("WorldManager::LoadTotemWorldFromText\n");
    ...
    World *tw = new World(); //perm;
    textArchive >> (*tw);
    hdPrintf("WorldManager::LoadTotemWorldFromText done\n");
    return tw;
}
#endif


World* WorldManager::LoadTotemWorld(const char* filename)
{
    /* std::iostream stuff */
    char *hfBuffer;
    long hfLen;
    ...
    World *tw = this->LoadTotemWorldFromUncompressedBinary(issBuffer, issLen);
#else
    World tw = this->LoadTotemWorldFromUncompressedBinary(issBuffer, issLen);
#endif
    
    WorldCache cacheItem;
...
#if RETURN_AS_POINTER == 1
static World *perm = new World();
World* WorldManager::LoadTotemWorldFromUncompressedBinary(const char *bin, unsigned long binLength)
{
    World *tw = new World(); //perm;
    hdPrintf("WorldManager::LoadTotemWorldFromUncompressedBinary\n");
...
int WorldManager::DestroyTotemWorld(const World *tw)
{
    for (int i = 0; i < m_cache->GetItemCount(); i++)
    {
    ...
    hdPrintf("WorldManager::DestroyTotemWorld could not find cache to destroy.\n");
    return 0;
}
