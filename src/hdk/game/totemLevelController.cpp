...
using namespace std;
#include "TotemLevelController.h"
...
TotemLevelController::TotemLevelController(const hdInterfaceController* parentController) :
...
TotemLevelController::~TotemLevelController()
{
...
void TotemLevelController::InitPhysics()
{
...
void TotemLevelController::InitAnimations() {}


void TotemLevelController::InitSounds()
{
...
void TotemLevelController::InitEffects()
{
...
void TotemLevelController::InitInterface()
{
    float longSide;
    float shortSide;
...
void TotemLevelController::RefreshLayout()
{
...
const bool TotemLevelController::InitLevel(const totemLevel* level, const int levelId)
{
    ...
    {
        event = (totemEvent *)m_currentLevel->GetEvents()->GetItems()[i];

        if (event->GetEventType() == e_totemEventTypeShowMessage)
        {
    ...
    {
        block = (totemBlock *)m_currentLevel->GetBlocks()[i];
    ...
    {
        joint = (totemJoint *)level->GetJoints()[i];
        if (joint->GetJointType() == e_totemJointTypeRevolute)
        {
            block = (totemBlock *)joint->GetFirstBody();
            if (block != NULL)
                block->AddGameFlag(e_totemGameFlagsBlockHasRevJoint);

            block = (totemBlock *)joint->GetSecondBody();
            if (block != NULL)
                block->AddGameFlag(e_totemGameFlagsBlockHasRevJoint);
        }
    }
    ...
    m_topBlock = new totemBlock(m_gameWorld,
                                m_physicsWorld,
    ...
    totemBlock* left = new totemBlock(m_gameWorld,
                                      m_physicsWorld,
    ...
    totemBlock* right = new totemBlock(m_gameWorld,
                                       m_physicsWorld,
    ...
    m_bottomBoundary = new totemBlock(m_gameWorld,
                                      m_physicsWorld,
    ...
    if (m_currentLevel->GetFloorInfo() != NULL)
    {
        ...
        {
            // Water is a hazard.
            m_floor = new totemBlock(m_gameWorld,
                                     m_physicsWorld,
    ...
    if (m_currBlock == NULL)
    {
        m_currBlock = (totemBlock *)FindBlockWithTag(1001);
    }
    ...
    {
        if (scriptMessages[i]->messageType == e_totemMessageTypeImage ||
            scriptMessages[i]->messageType == e_totemMessageTypeCustomImageText ||
            scriptMessages[i]->messageType == e_totemMessageTypeAvatar)
        {
...
void TotemLevelController::ResetCurrentLevel()
{
...
void TotemLevelController::RestartCurrentLevel()
{
...
totemBlock* TotemLevelController::FindBlockWithTag(const int tag) const
{
    totemBlock *b = NULL;
    ...
    for (int i = 0; i < m_currentLevel->GetBlockCount(); ++i)
    {
        ...
        {
            b = (totemBlock*)m_currentLevel->GetBlocks()[i];
            break;
        }
    }
    return b;
...
void TotemLevelController::PhysicsStep(double interval)
{
    float proportion;
...
void TotemLevelController::PanToSelectedObject(double interval)
{
    hdVec3 pan;
    if (m_currBlock != NULL &&
        m_interactionState != e_interactionDisabled &&
        m_currBlock->GetPhysicsBody() != NULL && m_currBlock->IsDraggable())
    {
        ...
        {
            if (m_currBlock->ContainsGameFlag((uint32)e_totemGameFlagsBlockDrawRaw))
            {
...
void TotemLevelController::Step(double interval)
{
...
void TotemLevelController::HandleCollision(const ContactPoint* point)
{
    ...
    if (point->state == e_contactAdded && b1->GetUserData() != NULL && b2->GetUserData() != NULL)
    {
        ...
        if (obj1->GetUserType() == (int)e_totemTypeBlock && obj2->GetUserType() == (int)kPhysicsParticleUserType)
        {
            HandleParticleBlockCollision((hdPhysicsParticle *)obj2, (totemBlock *)obj1, point);
        }
        else if (obj2->GetUserType() == (int)e_totemTypeBlock && obj1->GetUserType() == (int)kPhysicsParticleUserType)
        {
            HandleParticleBlockCollision((hdPhysicsParticle *)obj1, (totemBlock *)obj2, point);
        }
        else if (obj1->GetUserType() == (int)e_totemTypeBlock && obj2->GetUserType() == (int)e_totemTypeBlock)
        {
            HandleBlockBlockCollision((totemBlock *)obj1, (totemBlock *)obj2, point);

        }
        else if (point->state == e_contactAdded)
        {
            PlaySoftCollisionSound((totemBlock *)obj1, b1);
        }
    }
...
void TotemLevelController::HandleParticleBlockCollision(hdPhysicsParticle *particle, totemBlock *block, const ContactPoint *point)
{
    if (block == m_floor)
    {
    ...
    if (block->IsHazard() ||
        block->GetMaterial() == e_totemMaterialGrinder ||
        block->GetMaterial() == e_totemMaterialBomb)
    {
    ...
    if (particle->m_parentTag == (unsigned int)e_totemMaterialPlastic &&
        block->GetMaterial() == e_totemMaterialWood)
    {
        HandleParticleTap(particle, hdVec3(point->position.x, point->position.y, 0.0f), true);
    }
}
...
void TotemLevelController::HandleFragmentSplash(hdPhysicsParticle *fragment, const ContactPoint *point)
{
...
void TotemLevelController::HandleBlockBlockCollision(totemBlock *block1, totemBlock *block2, const ContactPoint *point)
{
    totemJoint *joint;
    hdAABB combinedAABB;
    unsigned i;
     ...
     */
    if (block1->ContainsGameFlag(e_totemGameFlagsBlockHasRevJoint) &&
        block2->ContainsGameFlag(e_totemGameFlagsBlockHasRevJoint))
    {
        // If there is a joint connect these two, then reject collision.
        for (i = 0; i < m_currentLevel->GetJointCount(); ++i)
        {
            joint = (totemJoint *)m_currentLevel->GetJoints()[i];
            if (joint->GetJointType() == e_totemJointTypeRevolute &&
    ...
    if (block1->IsGoal())
    {
        if (block2->GetMaterial() == e_totemMaterialJack)
        {
    ...
    if (block2->IsGoal())
    {
        if (block1->GetMaterial() == e_totemMaterialJack)
        {
            // Handle block to goal
    ...
    if (block1->IsHazard() || block1->GetMaterial() == e_totemMaterialGrinder)
    {
        if (block2->GetMaterial() != e_totemMaterialBomb &&
    ...
    if (block2->IsHazard() || block2->GetMaterial() == e_totemMaterialGrinder)
    {
        if (block1->GetMaterial() != e_totemMaterialBomb &&
    ...
    if (block1->GetMaterial() == e_totemMaterialPlastic)
    {
        if (block2->GetMaterial() != e_totemMaterialPlastic &&
            (block2->IsDestroyable() || block2->IsSpecial()))
        {
...
void TotemLevelController::PlayLoudCollisionSound(const totemBlock* block, const b2Body* body)
{
    float vol;
    hdSound *sound;
    ...
    if (playedLoudCollisionSound == false)
    {
        ...
        if (vol > 0.25f)
        {
            switch(((totemBlock *)block)->GetMaterial())
            {
...
void TotemLevelController::PlaySoftCollisionSound(const totemBlock* block, const b2Body* body)
{
...
const hdSound* TotemLevelController::GetCollisionSoundForMaterial(const e_totemMaterial material) const
{
    hdSound *sound = NULL;
...
bool TotemLevelController::AddBlockToCurrentLevel(totemBlock *block)
{
...
void TotemLevelController::PushProjectionMatrix()
{
...
void TotemLevelController::PopProjectionMatrix()
{
    hdAssert(m_pushedProjectionMatrix);
    glMatrixMode(GL_PROJECTION);
#if ROTATION_EXPERIMENT == 1
...
void TotemLevelController::DrawInternal()
{
    ...
    for (int i = 0; i < GetCurrentLevel()->GetBlockCount(); i++)
    {
        ...
        if (block->GetBlockType() == e_totemBlockTypeLight)
        {
...
void TotemLevelController::DrawInternalInterface()
{
...
void TotemLevelController::DrawSelectedBlockHighlight()
{
    float len;
 ...
 * to the code below.
 */
void TotemLevelController::HandleTapUp(float x, float y, int tapCount)
{
...
void TotemLevelController::HandleTapDown(float x, float y, int tapCount)
{
    hdVec2 screen;
    bool selected;
    ...
    else if (m_interactionState != e_interactionDisabled)
    {
        if (m_interactionState == e_waitingForInput)
        {
                ...
                m_currBlock->IsVisible())
            {
                ...
                Game_BlockWasSelected(m_currBlock);
                ...
                m_interactionState = e_interactionStateSelecting;
#ifdef DEBUG
                hdPrintf("[TotemLevelController::HandleTapDown] Block was selected.\n");
#endif
...
void TotemLevelController::HandleSingleTap(float x, float y)
{
    Game_HandleSingleTap(x, y);
}


void TotemLevelController::HandleDoubleTap(float x, float y)
{
    Game_HandleDoubleTap(x, y);
}


void TotemLevelController::HandleTapMovedSingle(const float previousX, const float previousY,
...
void TotemLevelController::HandleTapMovedDouble(const float aPreviousX, const float aPreviousY,
...
void TotemLevelController::HandleOrientationChanged()
{
    this->RefreshLayout();
...
bool TotemLevelController::SelectBlockAtScreenPoint(float x, float y)
{
    return SelectBlockAtScreenPoint(e_hdInterfaceClickStateUp, x, y);
}


bool TotemLevelController::SelectBlockAtScreenPoint(e_hdInterfaceClickState clickState, float x, float y)
{
    hdVec2 pp;
    bool particleFound;
    hdVec3 ray[2];
    ...
    if (clickState == e_hdInterfaceClickStateDown)
    {
        ...
        m_rayQueryCount = m_gameWorld->RayQuery((unsigned int)e_totemTypeBlock |
        ...
        for (int i = 0; i < hdMin(m_rayQueryCount, k_maxCount); ++i)
        {
            ...
            else if (objs[i]->GetUserType() == (int)e_totemTypeBlock &&
                     Game_CanSelectBlock((totemBlock*)objs[i]))
            {
                top = objs[i];
                break;
            }
    ...
    {
        m_rayQueryCount = m_gameWorld->RayQuery((unsigned int)e_totemTypeBlock, ray, objs, k_maxCount, m_testPoint);

        for (int i = 0; i < hdMin(m_rayQueryCount,k_maxCount); ++i)
        {
            if (objs[i]->GetUserType() == (int)e_totemTypeBlock)
            {
                if (Game_CanSelectBlock((totemBlock*)objs[i]))
                {
                    m_currBlock = (totemBlock*)objs[i];
                    return true;
                }
            }
        }
...
void TotemLevelController::HandleBlockLoss(const hdVec3& contactPoint, totemBlock* block)
{
#ifdef DEBUG2
    hdPrintf("[I] TotemLevelController::HandleBlockLoss contact point at (%3.5f, %3.5f, %3.5f)", contactPoint.x, contactPoint.y, contactPoint.z);
#endif

    // A block that is already "lost" cannot be lost again.
    if (block->ContainsGameFlag(e_totemGameFlagsBlockLost)) return;
...
void TotemLevelController::HandleBlockWorldExit(const hdVec3& contactPoint, totemBlock* block)
{
    if (block->ContainsGameFlag(e_totemGameFlagsBlockLost))
    {
...
void TotemLevelController::CreateMouseJoint()
{
    ...
    if (m_currBlock->IsDraggable() && (NULL != m_currBlock->GetPhysicsBody()))
    {
        ...
        if (m_currBlock->GetMaterial() == e_totemMaterialBomb)
        {
...
void TotemLevelController::UpdateMouseJoint(float x, float y)
{
...
void TotemLevelController::DestroyMouseJoint()
{
    if (m_mouseJoint == NULL) return; // callers responsible for checking first.
...
void TotemLevelController::HandleSenderBlockTapDownEvent(const totemBlock* sender, totemEvent* ev)
{
    totemJoint *receiver;
    switch(ev->GetEventType())
    {
        case e_totemEventTypeFireGun:
            break;
        case e_totemEventTypeShowMessage:
            break;
        case e_totemEventTypeJointMotorStartPos:
            if (ev->GetSecondObject()->GetUserType() == (int)e_totemTypeJoint)
            {
                receiver = (totemJoint *)ev->GetSecondObject();
                JointMotorStart(receiver, true);
            }
            break;
        case e_totemEventTypeJointMotorStartNeg:
            if (ev->GetSecondObject()->GetUserType() == (int)e_totemTypeJoint)
            {
                receiver = (totemJoint *)ev->GetSecondObject();
            ...
            break;
        case e_totemEventTypeJointMotorStop:
...
void TotemLevelController::HandleSenderBlockTapUpEvent(const totemBlock* sender, totemEvent* ev)
{
    switch(ev->GetEventType())
    {
        case e_totemEventTypeFireGun:
        {
            ...
            totemBlock *receiver = (totemBlock *)ev->GetSecondObject();
            ...
            totemBlock *block = new totemBlock(m_gameWorld, m_physicsWorld, aa, bb, e_totemMaterialMetal,
                                               e_totemShapeTypeCylinder, e_totemBlockTypeHazardDest);
        ...
        case e_totemEventTypeJointMotorStop:
        {
            totemJoint *receiver = (totemJoint *)ev->GetSecondObject();
...
void TotemLevelController::ShowMessage()
{
    float longSide;
    float shortSide;
...
void TotemLevelController::HideMessage()
{
    float longSide;
    float shortSide;
    ...
    anim->SetFinishedCallback(this, TotemLevelController::HideMessageCallback);
    anim->StartAnimation();
...
void TotemLevelController::ShowMessageInterface(hdUIContainer *container, hdUIContainer *buttonContainer,
...
void TotemLevelController::HideMessageInterface(hdUIContainer *container, hdUIContainer *buttonContainer,
                                                const float screenWidth, const float screenHeight)
{
    hdAnimation* anim1 = hdAnimationController::CreateAnimation(this);
    hdAnimation* anim2 = hdAnimationController::CreateAnimation(this);
    ...
    anim1->AddGameObject(container);
    anim1->AddAction(action1);
    //anim1->SetFinishedCallback(this, TotemLevelController::HideMessageCallback);
    anim1->StartAnimation();
...
void TotemLevelController::HideMessageCallback(void *handler, hdAnimation *anim)
{
    ...
    TotemLevelController *self = (TotemLevelController *)handler;
...
void TotemLevelController::SetMessageTexture(const char *texturePrefix)
{
...
void TotemLevelController::SetMessage(const hdTexture *texture)
{
...
void TotemLevelController::SetMessage(const totemMessage *msg)
{
    m_uiLandscapeAvatar->Hide();
    m_uiLandscapeAvatarChrome->Hide();
    m_uiPortraitAvatar->Hide();
    ...
    if (msg->messageType == e_totemMessageTypeImage)
    { 
    ...
    else if (msg->messageType == e_totemMessageTypeTitle)
    {
    ...
    else if (msg->messageType == e_totemMessageTypeText)
    {
    ...
    else if (msg->messageType == e_totemMessageTypeCustomImageText)
    {
        SetMessageTexture(msg->texture);
    ...
    else if (msg->messageType == e_totemMessageTypeAvatar)
    {
...
void TotemLevelController::HandleBlockTap(totemBlock* block, const hdVec3& impactPoint, bool hazardCollision)
{
     ...
     */
    if (block->GetMaterial() == e_totemMaterialBomb) return;
...
void TotemLevelController::HandleParticleTap(hdPhysicsParticle* particle, const hdVec3& impactPoint, bool hazardCollision)
{
    polygon_info_t polygonInfo;
...
const bool TotemLevelController::IsShowingMessage() const
{
...
void TotemLevelController::LoadScriptMessageForTag(const int tag)
{
...
void TotemLevelController::LoadScriptMessageForTag(const int tag, const bool addToQueue)
{
    LoadScriptMessageForTagAndLevel(tag, m_currentLevelId, addToQueue);
}


void TotemLevelController::LoadScriptMessageForTagAndLevel(const int tag, const int levelId,  const bool addToQueue)
{
    int msgCount;
    totemMessage t;
...
void TotemLevelController::LoadScriptMessageForTagAndLevel(const int tag, const int levelId, const bool addToQueue, const bool noShow)
{
    m_currentMessageIndex = 0;
    m_messageCount = Scripts_GetLevelMessagesForTag(m_scriptMessages, 10, levelId, tag);
}
...
const bool TotemLevelController::ShowNextScriptMessage()
{
    m_OkButton->Hide();
    m_PrevButton->Enable();
    if (m_messageCount <= 1)
...
const bool TotemLevelController::Game_InitLevel(const totemLevel *level, const int levelId)
{
    return true;
}


void TotemLevelController::Game_Step(double interval) {}


void TotemLevelController::Game_ResetCurrentLevel() {}


const bool TotemLevelController::Game_CanSelectBlock(const totemBlock *block)
{
    return (!block->IsStatic());
}


void TotemLevelController::Game_BlockWasSelected(const totemBlock *block) const {}


const bool TotemLevelController::Game_HandleTapUp(float x, float y, int tapCount)
{
    return false;
}


const bool TotemLevelController::Game_HandleTapDown(float x, float y, int tapCount)
{
    return false;
}


const bool TotemLevelController::Game_HandleSingleTap(float x, float y)
{
    return false;
}


const bool TotemLevelController::Game_HandleDoubleTap(float x, float y)
{
    return false;
}


const bool TotemLevelController::Game_HandleTapMovedSingle(const float previousX, const float previousY, 
                                                           const float currentX, const float currentY)
{
    return false;
}
...
const bool TotemLevelController::Game_HandleTapMovedDouble(const float aPreviousX, const float aPreviousY,
                                                           const float aCurrentX, const float aCurrentY,
                                                           const float bPreviousX, const float bPreviousY,	
                                                           const float bCurrentX, const float bCurrentY)
{
...
const bool TotemLevelController::Game_HandleBlockTap(totemBlock *block, const hdVec3& impactPoint, bool hazardCollision)
{
    return true;
}


const bool TotemLevelController::Game_HandleParticleTap(hdPhysicsParticle* particle, const hdVec3& impactPoint, bool hazardCollision)
{
    return true;
}


const bool TotemLevelController::Game_HandleBlockLoss(totemBlock* block, const hdVec3& contactPoint) 
{
    return false;
}


void TotemLevelController::Game_HandleCollision(const ContactPoint* point) {}


void TotemLevelController::Game_ShowMessage() {}


void TotemLevelController::Game_HideMessage() {}


void TotemLevelController::Game_PlayLoudCollisionSound(const totemBlock* block, const b2Body* body) {}


void TotemLevelController::Game_PlaySoftCollisionSound(const totemBlock* block, const b2Body* body) {}


void TotemLevelController::Game_AddBlockToLevel(const totemBlock* block) {}


void TotemLevelController::Game_HandleOrientationChanged() {}


void TotemLevelController::Game_HandleGoalCollision(totemBlock *goalBlock, totemBlock *block, const hdVec3& impactPoint) {}