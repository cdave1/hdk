...
public:
    ...
    FloorInfo(const char *firstTextureName, const char *secondTextureName, const hdVec3& flowVector, float floorLevel, bool isReflective);
...
BOOST_CLASS_VERSION(FloorInfo, 1)


inline const bool FloorInfo::IsDrawable() const
{
    return (((FloorInfo *)this)->GetFirstTexture() != NULL && m_isWater);
}


#ifdef LEVEL_EDITOR
inline void FloorInfo::ResetTextures()
{
    m_firstTexture = m_secondTexture = NULL;
}
#endif

inline hdTexture* FloorInfo::GetFirstTexture()
{
...
inline hdTexture* FloorInfo::GetSecondTexture()
{
