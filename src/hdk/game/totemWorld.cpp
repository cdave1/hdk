...
totemWorld::totemWorld()
{
    m_levelCount = 0;
    m_name = "";
    memset(m_levels, 0, sizeof(m_levels));
    hdPrintf("###### Creating totem world\n");
}


totemWorld::totemWorld(const char* name)
{
    m_levelCount = 0;
    m_name = name;
    memset(m_levels, 0, sizeof(m_levels));
...
int totemWorld::AddLevel(const totemLevel* level)
{
...
int totemWorld::AddLevelAtIndex(const totemLevel* level, const int index)
{
    hdAssert(m_levelCount >= 0 && m_levelCount <= kMaxTotemWorldLevels);
    ...
    m_levels[index] = (totemLevel *)level;
    ++m_levelCount;
    return index;
}


bool totemWorld::RemoveLevelAtIndex(const int index)
{
    hdAssert(m_levelCount >= 0 && m_levelCount <= kMaxTotemWorldLevels);
