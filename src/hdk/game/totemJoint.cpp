...
totemJoint::totemJoint() : hdPhysicsJoint(), hdPolygon()
{
    m_userType = (int)e_totemTypeJoint;
    m_joint = NULL;
...
totemJoint::totemJoint(hdGameWorld *gameWorld,
                       b2World *physicsWorld,
                       e_totemJointType jointType,
                       const hdVec3& rotationPoint,
                       totemBlock* firstBody,
                       totemBlock* secondBody) : hdPhysicsJoint(), hdPolygon(gameWorld)
{
    m_userType = (int)e_totemTypeJoint;
    m_joint = NULL;
    ...
    switch (jointType) {
        case e_totemJointTypeRevolute:
            m_colorTint[0] = 1.0f;
            m_colorTint[1] = 0.6f;
            m_colorTint[2] = 0.0f;
            m_colorTint[3] = 1.0f;
            break;
        case e_totemJointTypeDistance:
            m_colorTint[0] = 0.0f;
            m_colorTint[1] = 0.6f;
            m_colorTint[2] = 1.0f;
            m_colorTint[3] = 1.0f;
            break;
        case e_totemJointTypePrismatic:
            m_colorTint[0] = 1.0f;
            m_colorTint[1] = 1.0f;
            m_colorTint[2] = 0.0f;
            m_colorTint[3] = 1.0f;
...
totemJoint::~totemJoint()
{
}


void totemJoint::Init(hdGameWorld *gameWorld,
                      b2World *physicsWorld)
{
...
void totemJoint::CreateJoint()
{
    ...
    if (m_jointType == e_totemJointTypeRevolute)
    {
    ...
    else if (m_jointType == e_totemJointTypePrismatic)
    {
    ...
    else if (m_jointType == e_totemJointTypeDistance)
    {
...
const int totemJoint::SetFirstBody(const totemBlock* body)
{
    if (body == m_secondTotemBlock) return -1;
    if (body == NULL) return -1;
    m_firstTotemBlock = (totemBlock*)body;
    return 0;
}


const int totemJoint::SetSecondBody(const totemBlock* body)
{
    if (body == m_firstTotemBlock) return -1;
    if (body == NULL) return -1;
    m_secondTotemBlock = (totemBlock*)body;
    m_secondBodyPoint = ((totemBlock*)body)->GetWorldCenter();
    return 0;
}


void totemJoint::SetSecondJointPoint(const hdVec3& point)
{
    m_secondBodyPoint = point;
}


void totemJoint::Step()
{
...
bool totemJoint::AABBIntersection(const hdAABB& aabb)
{
#ifdef LEVEL_EDITOR
    hdAABB depthAABB;
    ...
    if (m_jointType == e_totemJointTypeDistance)
    {
        hdAABB depthAABB;
 ...
 */
void totemJoint::Scale(const hdVec3& scaleVector, const hdVec3& scaleAxis)
{
    if (m_jointType == e_totemJointTypeDistance)
    {
...
void totemJoint::Draw() const
{
    ...
    if (m_jointType == e_totemJointTypeDistance || m_jointType == e_totemJointTypePrismatic)
    {
        if (m_joint != NULL && m_firstTotemBlock->GetPhysicsBody() != NULL && m_secondTotemBlock->GetPhysicsBody() != NULL)
        {
            ...
            if (m_jointType == e_totemJointTypePrismatic)
            {
    ...
    else if (m_jointType == e_totemJointTypeRevolute)
    {
        hdglBegin(GL_LINES);
        hdglColor4f(0.9f, 0.9f, 0.0f, 1.0f);

        center = ((totemJoint *)this)->GetWorldCenter();
        firstCenter = ((totemBlock *)((totemJoint *)this)->GetFirstBody())->GetWorldCenter();
        secondCenter = ((totemBlock *)((totemJoint *)this)->GetSecondBody())->GetWorldCenter();
...
void totemJoint::InternalDrawJoint(float x, float y, float z) const
{
