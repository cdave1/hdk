...
{
    if (polygon->IsHidden()) return;
    DrawVerticesWithTint(polygon->GetVertices(), polygon->GetTextureCoordinates(),
                         polygon->GetVertexCount(), polygon->GetAABB(), texture, colorTint, alpha);
}


void DrawExtrudedPolygonWithTint(hdPolygon* polygon, hdTexture* texture, const float* colorTint,
                                 const float alpha, const float depth, const hdVec3& screenCenter)
{
    if (polygon->IsHidden()) return;
    DrawVerticesWithTint(polygon->GetVertices(), polygon->GetTextureCoordinates(),
                         polygon->GetVertexCount(), polygon->GetAABB(), texture, colorTint, alpha);
    ...
    DrawExtrusion(polygon->GetVertices(), polygon->GetTextureCoordinates(),
                  polygon->GetVertexCount(), polygon->GetAABB(), texture, colorTint, alpha, depth,
...
void DrawVertices(hdVec3* vertices, hdVec2* textureCoordinates, const int vertexCount,
                  const hdAABB& aabb, hdTexture* texture)
{
