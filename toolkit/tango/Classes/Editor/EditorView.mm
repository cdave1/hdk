		   ...
		   depthBits:(int)numDepthBits fullscreen:(BOOL)runFullScreen
{
	...
	[self handleResize];
	[InterfaceBuilder sharedInstance]->Resize(frame.size.width, frame.size.height);
...
{
	LOCK_GL_CONTEXT;
	[InterfaceBuilder sharedInstance]->Draw();
...
{
	LOCK_GL_CONTEXT;
	[InterfaceBuilder sharedInstance]->Resize(self.frame.size.width, self.frame.size.height);
	[super reshape];
	UNLOCK_GL_CONTEXT;
}
...
- (void)rightMouseDown:(NSEvent *)theEvent
{
	...
	[InterfaceBuilder sharedInstance]->StartPan(theEvent.locationInWindow.x, theEvent.locationInWindow.y);
...
{
	LOCK_GL_CONTEXT;
	[InterfaceBuilder sharedInstance]->Pan(theEvent.deltaX, -theEvent.deltaY);
...
{
	uint32 selectedCount = [InterfaceBuilder sharedInstance]->GetSelectedInterfaceObjectsCount();
	
	[[NSNotificationCenter defaultCenter] 
	 postNotificationName:kSingleItemWillBeUnselectedNotification
	 object:nil];
	
	[InterfaceBuilder sharedInstance]->MouseDown(theEvent.locationInWindow.x, theEvent.locationInWindow.y);
	
	if (([InterfaceBuilder sharedInstance]->GetSelectedInterfaceObjectSingle()))
	{
		[[NSNotificationCenter defaultCenter] 
	...
	{
		[[NSNotificationCenter defaultCenter] 
		 postNotificationName:kSingleItemWasUnselectedNotification
		 object:nil];
	}
}
...
- (void)mouseDragged:(NSEvent *)theEvent
{
	...
	{
		[InterfaceBuilder sharedInstance]->SetDragStyleScale();
	}
	else if (([theEvent modifierFlags] & NSControlKeyMask) == NSControlKeyMask)
	{
		[InterfaceBuilder sharedInstance]->SetDragStyleRotate();
	}
	else 
	{
		[InterfaceBuilder sharedInstance]->SetDragStyleMove();
	}
	
	[InterfaceBuilder sharedInstance]->MouseDrag(theEvent.locationInWindow.x, theEvent.locationInWindow.y);
}
...
- (void)mouseUp:(NSEvent *)theEvent
{
	...
	[InterfaceBuilder sharedInstance]->MouseUp(shiftKeyDown);
	
	if ([InterfaceBuilder sharedInstance]->GetSelectedInterfaceObjectsCount() > 0)
	{
		if (([InterfaceBuilder sharedInstance]->GetSelectedInterfaceObjectSingle()))
		{
			[[NSNotificationCenter defaultCenter] 
...
- (void)scrollWheel:(NSEvent *)theEvent
{
	...
	{
		[InterfaceBuilder sharedInstance]->Zoom([theEvent deltaY]);
